...
const EventModal: React.FC<EventModalProps> = ({
  ...
  const handleMoreOptions = () => {
    if (event?.id) {
      router.push(`/eventedit/${event.id}`);
    } else {
      const finalAllDay = showTimeInputs ? allDay : true;
      const searchParams = new URLSearchParams({
        ...
        allDay: String(finalAllDay)
      });
      router.push(`/eventedit?${searchParams.toString()}`);
    }
    onClose();
  };
      ...
      <DialogContent sx={{ px: 3, pt: 2, pb: 1, overflowY: 'auto', overflowX: 'hidden', maxHeight: '500px' }}>
        <Box sx={{ pl: 5, mb: 1.5 }}>
          <TextField
            ...
            slotProps={{ input: { disableUnderline: false, sx: { fontSize: '1.375rem', color: '#3c4043', '&::before': { borderBottom: '2px solid transparent !important' }, '&:hover::before': { borderBottom: '2px solid #dadce0 !important' }, '&::after': { borderBottom: '2px solid #1a73e8 !important' } } } }}
          />
        </Box>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                ...
                <>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'nowrap', width: 'max-content' }}>
                    <DatePicker 
                      ...
                      format="EEEE, d MMMM" 
                      slotProps={{ textField: { variant: 'standard', slotProps: { input: { disableUnderline: true, sx: { fontSize: '0.875rem', backgroundColor: '#f1f3f4', px: 1, py: 0.5, borderRadius: '4px', cursor: 'pointer', '&:hover': { backgroundColor: '#e8eaed' } } } } } }} 
                    />
                    ...
                    {allDay && (
                      <>
                        ...
                        <DatePicker value={endDate} onChange={(val) => { if(val) setEndDate(val); }} format="EEEE, d MMMM" slotProps={{ textField: { variant: 'standard', slotProps: { input: { disableUnderline: true, sx: { fontSize: '0.875rem', backgroundColor: '#f1f3f4', px: 1, py: 0.5, borderRadius: '4px', cursor: 'pointer', '&:hover': { backgroundColor: '#e8eaed' } } } } } }} />
                      </>
                    )}
                  </Box>
          ...
          {/* Menus for dropdowns */}
          <Menu anchorEl={colorAnchorEl} open={Boolean(colorAnchorEl)} onClose={() => setColorAnchorEl(null)} slotProps={{ paper: { sx: { p: 1, borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' } } }}>
          ...
          <Menu anchorEl={calendarAnchorEl} open={Boolean(calendarAnchorEl)} onClose={() => setCalendarAnchorEl(null)} slotProps={{ paper: { sx: { width: 220, borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' } } }}>
