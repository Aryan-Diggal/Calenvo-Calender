...
function EventEditPageContent() {
  ...
  const initialStartTimeStr = searchParams.get('startTime');
  const initialEndTimeStr = searchParams.get('endTime');
  const initialAllDayStr = searchParams.get('allDay');

  const [startDate, setStartDate] = useState<Date>(initialStartTimeStr ? new Date(initialStartTimeStr) : getDefaultStart());
  const [endDate, setEndDate] = useState<Date>(initialEndTimeStr ? new Date(initialEndTimeStr) : getDefaultEnd());
  const [startTime, setStartTime] = useState<Date>(initialStartTimeStr ? new Date(initialStartTimeStr) : getDefaultStart());
  const [endTime, setEndTime] = useState<Date>(initialEndTimeStr ? new Date(initialEndTimeStr) : getDefaultEnd());
  const [allDay, setAllDay] = useState(initialAllDayStr === 'true');
    ...
    <Box sx={{ minHeight: '100vh', backgroundColor: '#ffffff', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ display: 'flex', px: 8, py: 1, mt: 1, maxWidth: 1200, width: '100%', boxSizing: 'border-box' }}>
        <Box sx={{ flex: 1, display: 'flex', alignItems: 'center' }}>
          <IconButton onClick={() => router.push('/')} sx={{ color: '#5f6368', mr: 2, ml: -6 }}>
            <CloseIcon />
          </IconButton>
...
export default function EventEditPage() {
  return (
    <Suspense fallback={<Box sx={{ minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}><CircularProgress /></Box>}>
